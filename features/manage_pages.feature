フィーチャ: ページ管理
  ...
  背景:
    ...
    かつ   Wiki"b_note"のページ"FrontPage"を表示している
    ...
    かつ   "page_content_html"に"これはテストページです"と入力する
    かつ   "分類"から"Labelindice"を選択する
    かつ   "ページを作成"ボタンをクリックする

  シナリオ: ページ作成
    もし   Wiki"b_note"のページ"page_1"を表示している
  ...
  #  前提   Wiki"b_note"のページ"FrontPage"を表示している
  #  かつ   "ログアウト"リンクをクリックする
...
#     ならば "bob"と表示されていないこと
  ...
  シナリオ: ページの編集
    ...
    かつ "history_content"に"これはテストページです"と入力する
  ...
  シナリオ: FrontPage以外のページを公開/未公開を切り替える
    前提 Wiki"b_note"のページ"page_1"を表示している
    もし "公開にする"ボタンをクリックする
